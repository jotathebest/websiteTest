...
class AwsStorage(Storage):
    ...
    def open(self, file_url, timeout=60):
        ...
        """
        if not self.exist(file_url):
            message = "[ERROR] The file from path {}".format(file_url)
            message = "{} does not exist".format(message)
            raise AwsStorageError(message)
    ...
    def exist(self, file_url=None, aws_bucket=None):
        """
...
class AwsStorageError(ValueError):
    pass